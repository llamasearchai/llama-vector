...
class HNSWParams(BaseModel):
    ...
    M: int = Field(
        default=16, description="Number of connections per element in the index"
    )
...
class IVFParams(BaseModel):
    ...
    nprobe: int = Field(
        default=10, description="Number of clusters to visit during search"
    )
...
class QuantizationParams(BaseModel):
    ...
    enabled: bool = Field(
        default=False, description="Whether to enable product quantization"
    )
    pq_m: int = Field(default=8, description="Number of sub-quantizers for PQ")
    pq_nbits: int = Field(
        default=8, description="Number of bits per sub-quantizer (usually 8)"
    )
